...
async function runNewVerification() {
  console.log('=== New CSV Importer Verification Script (Native Fetch) ===');
  ...
  try {
    ...
    // 2. Read new CSV file
    const csvPath = path.join(__dirname, '../new_expenses_export.csv');
    ...
    formData.append('file', blob, 'new_expenses_export.csv');
    
    console.log('\nUploading new CSV file to /api/import/parse...');
    ...
    anomalies.forEach(row => {
      ...
      const hasAnomaly = (type) => row.anomalies.some(a => a.type === type);
      
      if (hasAnomaly('duplicate')) {
      ...
      } else if (hasAnomaly('missing_payer')) {
      ...
      } else if (hasAnomaly('alternate_name')) {
      ...
      } else if (hasAnomaly('non_group_member')) {
      ...
      } else if (hasAnomaly('settlement_entry')) {
      ...
      } else if (hasAnomaly('invalid_percentage')) {
      ...
      } else if (hasAnomaly('zero_amount')) {
      ...
      } else if (hasAnomaly('ambiguous_date')) {
        console.log(`  => Resolution: Select May 4th, 2026`);
        // Extract first choice date format
        const dateChoice = row.anomalies.find(a => a.type === 'ambiguous_date').choices?.[0] || '2026-05-01';
        resolutions.push({
          rowNumber: rowNum,
        ...
        resolutions.push({
          rowNumber: rowNum,
          issueType: row.anomalies[0].type,
          action: 'auto_fix'
        });
      }
    });
